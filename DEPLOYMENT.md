...
Снаружи должен быть открыт один входной адрес:

- домен, например `admin.company.ru`
- или IP, например `178.72.165.202`
...
- `Caddy` принимает входной адрес админки
- если используется домен, `Caddy` сам получает и продлевает SSL
- если используется только IP, работаем по `http://`, без автоматического SSL
...
1. Сервер в Selectel
2. Публичный IP сервера
3. Если будет домен: домен или поддомен для админки и доступ к DNS
...
## Как настроить входной адрес

Если используется домен, в DNS у заказчика нужно создать `A`-запись:
...
Если работаете только по IP:

- DNS не нужен
- в `.env` надо указать IP в `CADDY_SITE_ADDRESS`
- использовать `http://IP`, без `https`
